...
function compare(ex,pdf){
  if(!ex)return {status:"MISMATCH",checks:[],issues:["PO not found in Excel"],warnings:[]};
  const checks=[
  ...
  return {status:checks.every(x=>x.ok)&&!issues.length?"PASS":"MISMATCH",checks,issues,warnings};
}
...
function exportXlsx(){
  const summary=results.map(r=>{const fn=r.comp.checks.find(x=>x.field==="Filename");return {"PO":r.pdf.po_no,"Vendor Name":r.pdf.vendor,"Vendor Address (PDF)":r.pdf.vendor_address,"Ship-To":r.pdf.ship_to,"Shipping Address (PDF)":r.pdf.shipping_address,"Excel Qty":r.ex?.qty??"","PDF Qty":r.pdf.qty,"PO/Filename":fn?.ok?"PASS":"MISMATCH","Duplicate":r.duplicate?"YES":"NO","Result":r.comp.status,"Issues":[...r.comp.issues,...r.comp.checks.filter(x=>!x.ok).map(x=>x.detail)].join(" | "),"Warnings":r.comp.warnings.join(" | "),"Filename":r.pdf.filename}});
